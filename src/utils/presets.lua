...
local function getPresetDirectories()
	local dirs = {}
	local userDir = paths.USERDATA_THEME_PRESETS_DIR
	if userDir then
		table.insert(dirs, userDir)
	end
	table.insert(dirs, paths.THEME_PRESETS_DIR)
	return dirs
end
...
function presets.savePreset(presetName)
	...
	local presetsDir = paths.USERDATA_THEME_PRESETS_DIR
...
function presets.loadPreset(presetName)
	...
	logger.debug("Loading batteryActive field")
	loadField(loadedPreset, presetName, "batteryActive", function(v)
		state.setColorValue("batteryActive", v)
	end, "#4ADE80", "string")
	logger.debug("Loading batteryLow field")
	loadField(loadedPreset, presetName, "batteryLow", function(v)
		state.setColorValue("batteryLow", v)
	end, "#F87171", "string")
